...
int main(int argc,char*argv[]){
	if(argc<2){puts("Bitte geben Sie eine Konfigurationsdatei an.");return 1;}
	//Settings
	char*server;
	char*user;
	char*password;
	...
	yyin=fopen(argv[1],"r");
	while((current_token=yylex())!=0){//!=0 braucht man eigentlich nicht...
		switch(current_token){
			case servertoken:if(yylex()!=stringtoken){
					fprintf(stderr,"Ungueltiger Wert fuer Server:%s\n",yytext);
					return 1;
				}
				server=strdup(yytext+1);
				server[strlen(server)-1]='\0';
				break;
			case porttoken:if(yylex()!=numbertoken){
					fprintf(stderr,"Ungueltiger Wert fuer Port:%s\n",yytext);
					return 1;
				}
				port=atoi(yytext);
				break;
			case usertoken:if(yylex()!=stringtoken){
					fprintf(stderr,"Ungueltiger Wert fuer User:%s\n",yytext);
					return 1;
				}
				user=strdup(yytext+1);
				user[strlen(user)-1]='\0';
				break;
			case passwordtoken:switch(yylex()){
					case stringtoken:break;
					case passwordstringtoken:break;
					default:fprintf(stderr,"Ungueltiger Wert fuer Password:%s\n",yytext);return 1;break;
				}
				password=strdup(yytext+1);
				password[strlen(password)-1]='\0';
				break;
			case dbnametoken:if(yylex()!=stringtoken){
					fprintf(stderr,"Ungueltiger Wert fuer Database:%s\n",yytext);
					return 1;
				}
				database=strdup(yytext+1);
				database[strlen(database)-1]='\0';
				break;
			default:fprintf(stderr,"Ungueltiger Wert:%s\n",yytext);return 1;break;
		}
	}
	printf("Initialisiere mit: %s, %u, %s, %s, %s\n",server,port,user,password,database);
	puts("");
	//Mysql starts here
...
FROM `sdb_switches` WHERE `snmpEngineID`!='' AND (`snmp_auth_passphrase`!='' OR `snmp_priv_passphrase`!='')")){
		fprintf(stderr,"%s\n",mysql_error(conn_select));
		return 1;
	}
	...
	while((row=mysql_fetch_row(res))!=NULL){
		...
		update_counter+=mysql_affected_rows(conn_update);
		puts("\n");
	}
	printf("Insgesamt %llu Eintraege aufgelistet, %llu bearbeitet.\n\n",mysql_num_rows(res),update_counter);
	//Aufraeumen
	free(keyspace);
	free(auth_user_key);
	free(auth_local_key);
